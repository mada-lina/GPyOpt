...
class AcquisitionLCB_PSPACE(AcquisitionBase):
    """
    GP-Lower Confidence Bound acquisition function for binomial likelihood ()
    ...
    def __init__(self, model, space, optimizer=None, cost_withGradients=None, exploration_weight=2):
        ...
        super(AcquisitionLCB_PSPACE, self).__init__(model, space, optimizer)
    ...
    def _compute_acq(self, x):
        ...
        if type(self.model.model.likelihood.gp_link) != Identity:
            #pdb.set_trace()
    ...
    def _compute_acq_splitted(self, x):
        """
        Returns the mean and  std separately
        """
        m, s = self.model.predict(x)
        if type(self.model) == gpmodel.GPModelCustomLik:
            m, s = change_of_var_Phi(m, s)
