...
class GPModel(BOModel):
    ...
    def __init__(self, kernel=None, noise_var=None, exact_feval=False, optimizer='bfgs', 
                 ...
                 verbose=True, ARD=False, mean_function=None, heteroscedastic=False, 
                 binomial=False):
        self.kernel = kernel
        ...
        assert not(heteroscedastic and binomial), 'Heteroscedastic noise and binomial are uncompatible'
        self.heteroscedastic = heteroscedastic
        self.binomial = binomial
        
        # TODO: what is the rule for binomial
        if self.heteroscedastic or self.binomial:
            self.default_include_likelihood = False
        else:
            self.default_include_likelihood = True
    ...
    def _create_model(self, X, Y, **kwargs):
        ...
        if self.heteroscedastic:
            ...
            if self.exact_feval:
                ...
                self.model.sum.white.variance.constrain_bounded(1e-6, 1e6, warning=False) #constrain_positive(warning=False)
    
        
        elif self.binomial:
            kern += GPy.kern.White(self.input_dim)
            if getattr(self, '_add_bias', False):
                kern += GPy.kern.Bias(self.input_dim)
            # need to provide some metadata (the number of trials corresponding to each Y)
            #, the likelihood and inference method to use
            Y_Nrep = kwargs['Y_Nrep']
            Y_metadata= self._gen_YMetaData_binomial(Y_Nrep)
            likelihood = GPy.likelihoods.Binomial(gp_link=None)
            inf_method = GPy.inference.latent_function_inference.Laplace()
            self.model = GPy.core.GP(X, Y, kernel=kern, inference_method=inf_method, 
                                     likelihood=likelihood, normalizer=False, 
    ...
    def updateModel(self, X_all, Y_all, X_new, Y_new, update_hp=True, **kwargs):
        """
        ...
        if self.model is None:
            ...
            if self.heteroscedastic:
                ...
                Y_Nrep = kwargs['Y_Nrep']
                self.model.Y_metadata = self._gen_YMetaData_binomial(Y_Nrep)
    ...
    def predict_with_link(self, X):
        """
        Predictions with the model and link (only work for CDF or Identity links)
        Parameters:
            X (np.ndarray) - points to run the prediction for.
            with_noise (bool) - whether to add noise to the prediction. Default depends on the type of gp model.
        """

        m, v = self._predict(X, False, False)
    ...
    def predict(self, X, with_noise=None):
        """
        ...
        if with_noise is None:
            with_noise = self.default_include_likelihood
        m, v = self._predict(X, False, with_noise)
        # We can take the square root because v is just a diagonal matrix of variances
        return m, np.sqrt(v)

    def predict_covariance(self, X, with_noise=None):
        """
    ...
    def get_fmin(self):
        ...
        """
        return self.model.predict(self.model.X, include_likelihood=self.default_include_likelihood)[0].min()
    ...
    def predict_withGradients(self, X):
        ...
        m, v = self.model.predict(X, include_likelihood=self.default_include_likelihood)
        v = np.clip(v, 1e-10, np.inf)
